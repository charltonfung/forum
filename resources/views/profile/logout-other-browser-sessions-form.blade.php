<x-jet-action-section>
    ...
    <x-slot name="description">
        {{ __('管理並登出您在其他瀏覽器或裝置上的登入紀錄。') }}
    </x-slot>

    <x-slot name="content">
        <div class="max-w-xl text-sm text-gray-600">
            {{ __('必要時，您可以一次登出所有其他裝置上的瀏覽器。以下列出最近的部分登入紀錄（可能不完整）。如果懷疑帳號被盜，請同時更新密碼。') }}
        </div>
                            ...
                            <div>
                                <div class="text-xs text-gray-500">
                                    ...
                                    @if ($session->is_current_device)
                                        <span class="text-green-500 font-semibold">{{ __('目前裝置') }}</span>
                                    @else
                                        {{ __('最後活動於') }} {{ $session->last_active }}
                                    @endif
                                </div>
                            </div>
                        </div>
